<template>
  <div class="flex items-center gap-3 p-2 rounded-lg hover:bg-accented/50 transition-colors min-w-0 w-full h-18">
    ...
    <img :src="replaceImageUrl(props.playlist.artwork_url)" :alt="props.playlist.title" class="size-14 rounded-sm object-cover flex-shrink-0" />
    <div class="flex flex-col min-w-0 flex-1">
      ...
      <p class="truncate text-sm text-muted">{{ $t("skye.trackList.tracks", {
    ...
    <div class="flex items-center gap-2">
      <UButton color="neutral" :icon="user.isLikedSystemPlaylist(props.playlist.id) ? 'i-mingcute-heart-fill' : 'i-mingcute-heart-line'" variant="ghost" @click="user.toggleLikeSystemPlaylist(props.playlist.id)" />
    </div>
  </div>
</template>
