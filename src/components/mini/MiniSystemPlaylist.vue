<template>
  <div class="flex items-center gap-3 p-2 rounded-lg hover:bg-accented/50 transition-colors min-w-0 w-full">
    ...
    <div class="flex items-center gap-2">
      <UButton :icon="user.isLikedSystemPlaylist(props.playlist.id) ? 'i-mingcute-heart-fill' : 'i-mingcute-heart-line'" variant="ghost" @click="user.toggleLikeSystemPlaylist(props.playlist.id)" />
    </div>
  </div>
</template>
